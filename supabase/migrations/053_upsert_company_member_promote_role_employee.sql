-- Membros da equipe criados/atualizados pelo admin supremo devem ter role != client no profiles.
-- Antes: ON CONFLICT em upsert_company_member atualizava company_id mas mantinha role 'client'
-- (ex.: usuário que já tinha se cadastrado como cliente). O AppGuard bloqueia client na /app.
